...
export function getBookAction(
  ...
  if (reservationStatus === "WAITING") return { label: "已在預約隊列", disabled: true, tone: "disabled" as const };
...
export function bookActionClass(tone: "borrow" | "reserve" | "disabled") {
  if (tone === "reserve") return "bg-amber-500 text-white hover:bg-amber-600";
  if (tone === "disabled") return "disabled:cursor-not-allowed disabled:opacity-55";
  return "";
}
