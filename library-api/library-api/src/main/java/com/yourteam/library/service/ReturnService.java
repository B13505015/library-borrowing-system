...
public class ReturnService {
    ...
    public String returnBook(int recordId) {
        ...
        boolean updateReturnSuccess = borrowRecordRepository.updateReturnDate(recordId, returnedAt);
        ...
        // 若逾期，建立罰款紀錄
        double fineAmount = 0;
        long overdueDays = PenaltyService.calculateOverdueDays(record.getDueDate(), returnedAt);
        if (overdueDays > 0) {
            fineAmount = PenaltyService.calculateFine(overdueDays);
            penaltyRepository.upsertFineForBorrowRecord(
                    record.getUserId(),
                    recordId,
                    fineAmount,
                    "逾期歸還 " + overdueDays + " 天"
